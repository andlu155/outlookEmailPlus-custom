...
- `SCHEDULER_STANDALONE`
  是否将 APScheduler 放到独立进程（Issue #69 Phase 4）。默认 `false` 保持兼容；设为 `true` 后可安全提高 `GUNICORN_WORKERS`
- `GUNICORN_WORKERS` / `GUNICORN_THREADS` / `GUNICORN_TIMEOUT`
  Docker / `scripts/start-gunicorn.sh` 并发参数。未启用独立调度器时请保持 `GUNICORN_WORKERS=1`，避免定时任务重复
- `OAUTH_TOOL_ENABLED`
